...
class Product
{
    ...
    public function getCategory(SqlRequest $sqlRequest): ?Category
    {
        $categoryData = $sqlRequest->getCategoryById($this->category_id);

        if ($categoryData) {
            // Créez directement une instance de Category avec les données récupérées
            return new Category(
                $categoryData['id'],
                $categoryData['name'],
